    ...
    def m_writeT4Composition(self):
        ...
        for k in dic_composition.keys():
            ...
            for element in list_isotope:
                ...
                s_paramMaterialComposition = s_paramMaterialComposition + str(nameIsotope) + ' ' + str(abondanceIsotope) +' '
                print(s_paramMaterialComposition)
            f.write("%s %s %s %s %s %s \r" % (p_typeDensity, temperature,p_materialName,p_densityValue, p_numberOfIsotope, s_paramMaterialComposition ))
