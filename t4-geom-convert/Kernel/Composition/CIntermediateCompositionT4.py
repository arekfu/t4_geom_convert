...
class CIntermediateCompositionT4(object):
    ...
    def m_constructCompositionT4(self):
        '''
        :brief: method changing the tuple from CCompositionConversionMCNPToT4 in instance of the CVolumeT4 Class
        '''
        ...
        for key in dic_CompositionT4.keys() :
            ...
            for k in dic_cellMCNP.keys():
                if int(dic_cellMCNP[k].materialID) == key:
                    ...
                    if float(density)<0 :
                        typeDensityT4 = 'Density'
                    if float(density)>0 :
                        typeDensityT4 = 'Point Wise'
            dic_newCompositionT4[key] = CCompositionT4(typeDensityT4, 'm'+str(key), float(density), len(l_listeMaterialComposition), l_listeMaterialComposition)
