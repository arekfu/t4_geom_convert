...
class CCellConversion(object):
    ...
    def m_postOrderTraversalConversion(self,tree):
        ...
        if CTreeMethods().m_isInterSurface(tree):
            id , op = tree[0:2]
            children = tree[2:]
            
            tupEQUA = self.m_conversionEQUA(children, fictive=True)
            opT4, param, fict = tupEQUA 
            self.dictClassT4.__setitem__(id, CVolumeT4(opT4, param, fict))
            return id
        
        if CTreeMethods().m_isLeaf(tree):
            id = abs(tree) + self.i*10 
            tupEQUA = self.m_conversionEQUA([tree], fictive=True)
            opT4, param, fict = tupEQUA
            self.dictClassT4[id] = CVolumeT4(opT4, param, fict)
            return id
         
        
            
        else:
            id , op, left, right = tree
    ...
    def m_postOrderTraversalOptimisation(self, tree):
